...
\begin{figure}[t!]
\centering
\includegraphics[height=8cm]{nanomaly/sumList-steps-new.png}
\caption[A sequence of interactions with the trace of \texttt{sumList [1]}.]{
  A sequence of interactions with the trace of
  \texttt{sumList [1]}. The stuck term is red, in each node the redex is
  ...
  into the recursive \texttt{sumList []} call, which spawns a new ``thread''
...
\input{nanomaly/traversals_new.tex}
